...
import { Youtube, Plus, Download, Filter, Eye, MessageSquare, ExternalLink, Trash2, Play, BarChart3, TrendingUp, Users, Heart, ChevronDown, ChevronUp } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, ResponsiveContainer, Tooltip } from 'recharts';

const YouTubeImports: React.FC = () => {
  ...
  const [savedUrls, setSavedUrls] = useState([
    {
      ...
      analytics: {
        engagementRate: 13.5,
        testimonialsConverted: 10,
        sentimentBreakdown: { positive: 88, neutral: 10, negative: 2 },
        performanceData: [
          { day: 'Mon', engagement: 20 },
          { day: 'Tue', engagement: 35 },
          { day: 'Wed', engagement: 28 },
          { day: 'Thu', engagement: 42 },
          { day: 'Fri', engagement: 38 }
        ]
      }
    },
    {
    ...
    <div className="p-6 space-y-6 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 min-h-screen">
      ...
      <div className="flex items-center justify-between animate-fade-in">
        <div>
          ...
          <p className="text-gray-600 text-lg">Import testimonials from YouTube videos by adding URLs</p>
        </div>
        <button
          onClick={() => setShowAddForm(true)}
          className="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-200 hover:scale-105 shadow-lg flex items-center space-x-2"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 animate-slide-up" style={{ animationDelay: '100ms' }}>
        {[
          { label: 'Total URLs', value: savedUrls.length.toString(), icon: Youtube, color: 'red' },
      ...
      {showAddForm && (
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white/20 animate-slide-up">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Add YouTube Video URL</h3>
          ...
          <p className="text-sm text-gray-500 mt-2">
            Paste the URL of any YouTube video to scan for testimonials in the comments
          </p>
        </div>
      )}
            ...
            >
              <div className="flex items-start space-x-6">
                ...
                <div className="relative flex-shrink-0">
                  <img
                    src={urlData.videoData.thumbnail}
                    alt="Video thumbnail"
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-2 text-sm text-gray-600 mb-4">
                    <span className="font-mono bg-gray-100 px-2 py-1 rounded text-xs">{urlData.url}</span>
                    <button
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm mb-4">
                    ...
                    <div>
                      ...
                      <span className="ml-2 font-semibold">{urlData.videoData.views}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Likes:</span>
                      <span className="ml-2 font-semibold">{urlData.videoData.likes}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Comments:</span>
                      <span className="ml-2 font-semibold">{urlData.videoData.comments}</span>
                    </div>
                    <div>
                ...
                <div className="flex items-center space-x-2">
                  <button className="bg-red-50 hover:bg-red-100 text-red-700 px-4 py-2 rounded-lg text-sm font-semibold transition-colors border border-red-200">
                    Scan Now
                  </button>
                  <button 
                    onClick={() => toggleUrlAnalytics(urlData.id)}
                    className="bg-red-50 hover:bg-red-100 text-red-700 px-3 py-2 rounded-lg text-sm font-semibold transition-colors border border-red-200 flex items-center space-x-1"
                  >
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 animate-slide-up">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                    <div className="bg-red-50 rounded-lg p-3 text-center border border-red-200">
                      ...
                      <div className="text-lg font-bold text-red-900">{urlData.analytics.engagementRate}%</div>
                      <div className="text-xs text-red-700">Engagement</div>
                    </div>
                  ...
                  <div className="bg-white rounded-lg p-4 border border-gray-200">
                    ...
                    <ResponsiveContainer width="100%" height={150}>
                      <LineChart data={urlData.analytics.performanceData}>
                        ...
                        />
                        <Line type="monotone" dataKey="engagement" stroke="#FF0000" strokeWidth={3} dot={{ fill: '#FF0000', strokeWidth: 2, r: 4 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
      ...
      {savedUrls.length === 0 && (
        <div className="text-center py-16 animate-fade-in">
          ...
          <h3 className="text-2xl font-bold text-gray-900 mb-4">No YouTube URLs added yet</h3>
          <p className="text-gray-600 mb-8 max-w-md mx-auto">
            Add YouTube video URLs to start importing testimonials from comments.
          </p>
          <button
            onClick={() => setShowAddForm(true)}
            className="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-200 hover:scale-105 shadow-lg"
          >
            Add Your First URL
          </button>
        </div>
